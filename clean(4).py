...
Cleaning pipeline — each step is backed by the reviewed literature:
  ...
  7. Source attribution removal
       ↳ "Source: ..." lines are non-content boilerplate that can introduce
         data leakage (e.g. if real articles consistently cite known outlets
         while fake ones do not). Covered under the general principle of
         stripping "advertisements, navigation bars and other boilerplate"
         [68†L389-L390] and removing layout/byline elements [59†L300-L307].

  8. Author byline / photo credit removal
 ...
 10. Short-row filtering (< 10 words)
       ↳ NOT applied — rows are preserved regardless of length.
...
Steps deliberately EXCLUDED (not supported by the reviewed studies):
  ...
  - Dataset-specific Filipino boilerplate (Panoorin, Basahin, etc.)
...
# 7a. Source attribution lines  [59†L300-L307]
#     "Source: CNN Philippines", "Source: 1 | 2 | 3", etc.
#     Always trailing boilerplate — strip everything from "Source:" to end.
_RE_SOURCE = re.compile(
    r"\s*Sources?\s*:.*$",
    re.IGNORECASE | re.DOTALL,
)

# 7b. Author bylines and photo credits  [59†L300-L307]
...
def clean_article(text: str) -> str:
    ...
    # 7a. Remove source attribution lines ("Source: CNN Philippines", etc.)
    text = _RE_SOURCE.sub("", text)

    # 7b. Remove author bylines and photo credits
